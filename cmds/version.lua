events.TICK:register(function()
    if candler then
        candler.lib.setCommand("Candler", "version", {
            aliases = {"ver"},
            ...
            if not candler.cats[args[1]] then
                printJson('[{"text":"ERROR!", "color":"red", "bold":true}, {"text":" This category does not exist. Type \\\"' .. candler.config.prefix .. 'cats\\\" for a list of categories.", "color":"gray", "bold":false}]')
            else
        ...
        events.TICK:remove("reg.candler.ver")
    end
end, "reg.candler.ver")