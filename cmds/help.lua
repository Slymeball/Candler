events.TICK:register(function()
    if candler then
        candler.lib.setCommand("Candler", "help", {
            aliases = {"?"},
            ...
            for key, value in pairs(commandList) do
                combList[candler.config.prefix .. key] = value
            end
            ...
            for key, value in pairs(a) do
                table.insert(ba, candler.config.prefix .. value)
            end
            -- print(ba)
            ...
            elseif catList[string.lower(args[1])] then
                ...
                if tonumber(args[2]) then
                    ...
                    for i = (8*(tonumber(args[2])-1))+1, (8*(tonumber(args[2])-1))+8 do
                        ...
                        desc = string.gsub(desc, "\\", "\\\\")
                        desc = string.gsub(desc, "\"", "\\\"")
                        printJson('[{"text":"\n' .. candler.config.prefix  .. tempA[i] .. ': ","color":"gold"},{"text":"' .. desc .. '","color":"white"}]')
                        ::continue::
                    end
                else
        ...
        events.TICK:remove("reg.candler.help")
    end
end, "reg.candler.help")